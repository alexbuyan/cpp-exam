...
Это помогает сделать один общий интерфейс, заданный в базовом классе, а у каждого потомка будет своя конкретная реализация методов, нужная именно ему. При вызове будет проверяться фактический тип объекта, и будет вызываться соответствующая реализация (а не как с обычными функцями на стадии компиляции).
...
`dynamic_cast` позволяет проверить реальный (динамический) тип объекта, на который показывает ссылка/указатель. Почему может отличаться от выясненного на этапе компиляции: как раз из-за наследования. Вот лежит `Base*`, а по факту там может быть аж целый `Derived1`. В случае с указателями `dynamic_cast` возвращает либо `nullptr`, если конвертация прошла неуспешно, в другом случае возвращает указатель. Со ссылками аналогично, только при сбое выкидывает `std::bad_cast` (так как не бывает "нулевой ссылки")

`dynamic_cast` работает только если `Base` - полиморфный (потому что создается таблица виртуальных функций и в ней мы можем сравнить указатели). 
```C++
// Для указателей
struct Base {  
    virtual void foo() {};  
