...
Для хранения строк используется массив байт `char arr[N]`, где `N` - `длина строки + 1` (так как строки в стиле Си всегда должны заканчиваться 0 символом). Можем завести указатель на строчку и вывести ее
```C++
char arr[] = "Hello";
...
std::cout << str << "\n";
```
Если в конце не будет `\0` символа, и мы начнём вызывать всякие функции, которые ожидают этот ноль в конце, начнут его искать, не найдут, выйдут за границу массива - `UB`
...
На самом деле `c_str()` и `data()` с C++11 возвращают одно и то же. Просто до C++11 не было гарантии, что `data()` возвращает null-terminated string.

Важно понимать, что `c_str()` возвращает указатель на память, которой всё ещё владеет `std::string`
...
```
Такой свой конкатенатор вызывает `UB` (но если памяти хватало - то не будет), потому что никто не гарантировал нам, что в память после этой строки можно еще что-то записывать.
...
Может возникнуть `UB` (тоже самое переполнение буффера, что и в `gets`)
```C++
{
    char buf[5];
